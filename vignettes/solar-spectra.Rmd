...
---
title: "The G173-03 reference solar spectra"
author: "Taha Ahmed"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{The G173-03 reference solar spectra}
...
This dataframe is in wide format. We can convert it to tidy (long) format
for easier plotting with ggplot2:
...
And take a first look at the G137-03 reference solar spectra as included in
this dataset `photoec::ASTMG173`:
...
`sunlight.ASTM()` function with those above (should be an exact match if you
use the same wavelength vector).
...
the dataset and the function are pairwise equal:
...
the `photoec::sunlight.ASTM()` function **includes other derived properties**, namely,
absolute and relative cumulative irradiance (`irradiance` and `irradiance.fraction`),
spectral photon flux (`spectralphotonflux`) and absolute and relative cumulative
photon flux (`photonflux` and `photonflux.fraction`).

This allows us to easily explore not only spectral irradiance but also irradiance
(radiant power in kW m⁻²) and the distribution of cumulative radiant power by wavelength.
And in the same way, it allows us to explore spectral photon flux, photon flux,
and cumulative photon flux by wavelength:

```{r function-exploration, echo=FALSE, fig.cap=caption.functionexploration}
...
About 75% of the total irradiance (i.e., power) is contained below 1000 nm,
but only about 50% of the total photon flux. This makes sense, since
each lower-wavelength photon carries more energy.

```{r echo=FALSE, results="markup", size="tiny"}
...
Please note that the spectral irradiance and flux shown in the table are not summed
over the wavelength interval. That is, they are simply the value *at the shown wavelength*,
and not the spectral irradiance/flux summed over the wavelength interval (10 nm in this case).
<!-- Perhaps we should change this? -->
The calculated current density and STH values assume perfect
quantum efficiency, Faradaic efficiency and no band gap and are only meant
as an illustrative ceiling.



## Geometry of the AM1.5G reference

Let's consider the the geometry of the global tilt solar reference
spectrum (AM1.5G).

At this point I would like to extend my thanks to Zhang Yangdong's blog for providing
one of the few illustrations (outside of the scientific literature) demonstrating all
the angles (ha!) involved in the AM1.5G geometry.
His blog appears to have been [lost to cyberspace](https://blog.sciencenet.cn/blog-616448-889852.html), but the
[illustration itself](http://image.sciencenet.cn/album/201505/13/120558i594h5nz124gfj3c.jpg)
is still accessible via the blog's webhost.

Below is my own sketch of the AM1.5G geometry (absorber is a plane tilted at 37°,
with AM1.5 irradiation) created with TikZ (all angles are correct,
but lengths are only illustrative):

```{r sketch, results="markup", out.width="33%", echo=FALSE, fig.cap=caption.am15g.geometry}
...
At air mass 1.5 the zenith angle is 48.2°, and the solar altitude is
(the complementary angle) 41.8°. Note how the tilted absorber negates some of the projection effect
(in the sketch, the tilted plane absorbs six solar rays, whereas the horizontal plane absorbs only five).
That is, of course, the whole point of tilting the absorbing surface at higher altitudes.
37° roughly matches the average latitude of the contiguous United States, and air mass 1.5,
as we have seen, corresponds to a latitude of 48° which is representative of the temperate
zone wherein many industrialised cities around the world happen to be located.
...
+ https://en.wikipedia.org/w/index.php?title=Geographic_center_of_the_United_States&oldid=1159819200#Contiguous_United_States
+ https://www.nrel.gov/grid/solar-resource/spectra-am1.5.html
