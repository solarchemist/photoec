...
sunlight.Planck <- function(wavelength, temperature = 5778) {
   ...
   #'
   #' @description
   #' Calculate the spectral radiance for an ideal black-body using Planck's law
   #' with specified wavelength range and wavelength step-size
   #'
   #' @details
   #' Calculate the spectral radiance for an ideal black-body using Planck's law
   #' with specified wavelength range and wavelength step-size
   #'
   #' @param wavelength: vector of wavelengths in meter (lambda/metre)
   #' @param temperature: blackbody surface temperature in Kelvin (T/K)
   #' @examples
   #' sunlight.Planck(wavelength, temperature)
   ...
   theory <- 
      data.frame(wavelength = wavelength,
                 ...
                 Sun.spectralradiance.trapz = 
                    c(0, trapz(wavelength, spectralradiance)),
                 # radiance (total radiant power) units of \watt\per\square\metre
                 Sun.radiance = 
                    cumsum(c(0, trapz(wavelength, spectralradiance))),
                 # luminosity (total radiance times surface area) units of \watt
                 Sun.luminosity = 
                    sum(c(0, trapz(wavelength, spectralradiance))) * 
                 ...
                 E.spectralradiance.trapz = 
                    c(0, trapz(wavelength, factor.inversesquare * spectralradiance)),
                 E.radiance = 
                    cumsum(c(0, trapz(wavelength, 
                                      factor.inversesquare * spectralradiance))),
                 # total luminosity hitting Earth's surface (day-side)
                 E.luminosity = 
                    sum(c(0, trapz(wavelength, 
                                   factor.inversesquare * spectralradiance))) * 
