...
the dataset and the function are pairwise equal, as expected:
...
this package's `sunlight.ASTM()` function derives other properties, namely,
...
Quick reminder: AM0 is the extra-terrestrial irradiance (solar spectrum just outside
earth's atmosphere), and AM1.5G is the solar spectrum at sea-level at
mid-latitudes, and DNCS is the direct normal circumsolar spectrum which
excludes diffuse light.
In the following we will discuss only AM1.5G, unless otherwise noted.
...
over the wavelength step. That is, they are simply the value at the shown wavelength,
and not the spectral irradiance/flux summed over the wavelength step (10 nm in this case).
